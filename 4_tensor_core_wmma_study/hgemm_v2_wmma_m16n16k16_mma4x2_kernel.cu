...
// only 1 warp per block(32 threads), m16n16k16. A, B, C: all
// row_major.
...
__global__ void hgemm_wmma_m16n16k16_mma4x2_kernel(half *A, half *B, half *C,
    ...
    const int warp_m = warp_id / WMMA_TILE_N;    // 0 ~ 3
    const int warp_n = warp_id % WMMA_TILE_N;    // 0 ~ 1

    // block有256个线程，分别load s_a=64×16 和 s_b=16×32
    // 对于A，每个线程load 4个half，共256x4=1024=64×16个half
    // 对于B，每个线程load 2个half，共256x2=512 =16×32个half
    // s_a 64×16，每个线程load 4 half，每行4个线程，64行，共256个线程
    const int load_smem_a_m = tid / 4;           // 0 ~ 63，每4个线程load同一行
    const int load_smem_a_k = (tid % 4) * 4;     // 0,4,8,12，每个线程load 4个half，间隔为4
    // s_b 16×32，每个线程load 2 half，每行16个线程，16行，共256个线程
    const int load_smem_b_k = tid / 16;          // 0 ~ 15，每16个线程load同一行
    const int load_smem_b_n = (tid % 16) * 2;    // 0,2,4,6，每个线程load 2个half
    const int load_gmem_a_m = by * BM + load_smem_a_m;  // 全局内存A的行索引
    const int load_gmem_b_n = bx * BN + load_smem_b_n;  // 全局内存B的列索引
...
#pragma unroll
    for (int k = 0; k < NUM_K_TILES; ++k) { // K方向的tile循环
        int load_gmem_a_k = k * WMMA_K + load_smem_a_k;             // 全局内存A的列索引
        int load_gmem_a_addr = load_gmem_a_m * K + load_gmem_a_k;   // 全局内存A的地址
        int load_gmem_b_k = k * WMMA_K + load_smem_b_k;             // 全局内存B的行索引
        int load_gmem_b_addr = load_gmem_b_k * N + load_gmem_b_n;   // 全局内存B的地址

        // 64 bit(16×4) sync memory issues gmem_a -> smem_a
        LDST64BITS(s_a[load_smem_a_m][load_smem_a_k]) = (LDST64BITS(A[load_gmem_a_addr]));
        // 32 bit(16×2) sync memory issues gmem_b -> smem
...
void hgemm_wmma_m16n16k16_mma4x2(half *A, half *B, half *C, int M, int N,
    ...
    dim3 block(256);
    dim3 grid(div_ceil(N, WMMA_N * WMMA_TILE_N), div_ceil(M, WMMA_M * WMMA_TILE_M)); // grid(64, 8) for (2048, 512)

    hgemm_wmma_m16n16k16_mma4x2_kernel<WMMA_M, WMMA_N, WMMA_K, WMMA_TILE_M, WMMA_TILE_N>
